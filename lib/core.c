...
{
	return gpiod_chip_watch_event_read_multiple(chip, event, 1);
}
...
int gpiod_line_unwatch(struct gpiod_line *line)
{
	...
	ret = ioctl(line->chip->fd,
		    GPIO_GET_LINEINFO_UNWATCH_IOCTL, line->offset);
	if (ret == 0)
		chip->watched_lines--;
